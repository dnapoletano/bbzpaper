...
understood as a consequence two concurring causes.
On the one hand, the resummed logarithms, $\ln\frac{\mu_F^2}{\mu_b^2}$
become smaller as $\mu_b$ increases. On the other hand, the
fixed-order logarithms, $\ln\frac{\mu_b^2}{m_b^2}$,
which comes from the matching conditions, become larger. However,
these two effects cancel out only at fixed-order. The remaining
higher-order logarithms in the resummed calculation become smaller
and are not matched by anything else, as the constant terms
remain constant. As these higher-order are the principal cause
of the difference between the 4FS and both the 5FS and the FONLL
calculations, as they become smaller, the difference consequently reduces.
...
\footnote{Although one would be able to perform a, less accurate, FONLL-A
  calculation}.
%
%% Previous version
%% However, the corresponding 4FS contributions are only
...
over a pure 5FS computation of this term would be of the order of
$\sim 6\%$, which, relative to the total $b\bar{b}$ cross section of
Table~\ref{tab:res}, yields a $0.7\%$ modification,
to be compared
to the 5-6\%  impact of the FONLL-B improvement of the bottom fusion
contribution. Therefore, its FONLL improvement appears unnecessary. 
